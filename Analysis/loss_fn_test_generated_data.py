    ...
    for axis in [0, 1]:
        ...
        for j in range(20+20*axis):
            ...
            for i, loss_fn in enumerate(loss_fn_list):
                loss_map[i, j] = loss_fn(res, res0)

        if axis == 0:
            loss_map1 = ax1.imshow(loss_map, extent=[0, 20+20*axis, len(loss_fn_list), 1])
            ...
            loss_map2 = ax2.imshow(loss_map, extent=[0, 20+20*axis, len(loss_fn_list), 1])
...
def noise_plotter(res0, res, loss_fn_list):
    fig = plt.figure(figsize=(6, 6))
    ax1 = fig.add_subplot(1, 1, 1)
    #ax2 = fig.add_subplot(2, 1, 2)

    noise_map = np.zeros((len(loss_fn_list), 40))
    for j in range(40):
        res += (np.random.randn(20, 40)-0.5) * 0.3
        for i, loss_fn in enumerate(loss_fn_list):
            noise_map[i, j] = loss_fn(res, res0)

    noise_map1 = ax1.imshow(noise_map)
    ax1.set_title("Loss Map (Noise Addition)")
    ax1.set_xlabel("Noise Steps")
    ax1.set_ylabel("Loss Value")
    plt.colorbar(noise_map1, ax=ax1, fraction=0.026, pad=0.04)

    plt.show()

def constant_plotter(res0, res, loss_fn_list):
    fig = plt.figure(figsize=(6, 6))
    ax1 = fig.add_subplot(1, 1, 1)
    #ax2 = fig.add_subplot(2, 1, 2)
    ...
    noise_map = np.zeros((len(loss_fn_list), 40))
    for j in range(40):
        res_temp = res*(0.05 * j + 0)
        for i, loss_fn in enumerate(loss_fn_list):
            noise_map[i, j] = loss_fn(res_temp, res0)

    noise_map1 = ax1.imshow(noise_map)
    ax1.set_title("Loss Map (Constant Multiplier)")
    ax1.set_xlabel("Constant Multiplier Steps")
    ax1.set_ylabel("Loss Value")
    plt.colorbar(noise_map1, ax=ax1, fraction=0.026, pad=0.04)
...
def loss7(x, y):
    ...
    x_prop_final = x_prop_pos*np.max(y_prop)/np.max(x_prop) + x_prop_neg*np.min(y_prop)/np.min(x_prop)
...
plotter(res0, res, loss7)
rolling_plotter(res0, res, loss_list)
noise_plotter(res0, res, loss_list)
constant_plotter(res0, res, loss_list)
