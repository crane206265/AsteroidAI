...
class QValueClassifierNet(nn.Module):
...
def train_loop(dataloader, model, loss_fn, optimizer, train_loss, es:EarlyStopping):
    ...
    for X_train, y_train in dataloader:
        ...
        y_train = y_train.squeeze(dim=-1).type(torch.LongTensor) # only for classification model
...
def test_loop(dataloader, model, loss_fn, test_loss, epoch):
    ...
    with torch.no_grad():
        for X_test, y_test in dataloader:
            ...
            total += y_test.size(0)
            correct += (pred.cpu() == y_test).sum().item()
...
def classify_reward(data, split_ref=[-2, 2]):
    ...
    class_idx = np.zeros_like(data)

    for i in range(class_num-1):
        class_idx = class_idx + np.where(data > split_ref[i], 1, 0)
...
def test_img_show(i_img):
    ...
    ax2.clear()
    im2 = ax2.imshow(class_map_temp.T)
...
split_ref = [-2, 2]
data_RL_preset0[1:, -1:], class_num = classify_reward(data_RL_preset0[1:, -1:], split_ref=split_ref)
...
print(reward_data)
print(np.count_nonzero(reward_data == 2))


model = QValueClassifierNet(input_dim=299, hidden_dim=512, output_dim=class_num, activation=nn.ReLU, dropout=0.15).to(device)
summary(model, (1, model.input_dim))

class_weight = torch.tensor([100, 10, 100]).float().to(device)
